...
void ezBindAppInitCallback(void (*init)()) {
    if (app == NULL) {
        EZ_DEBUGC_RAW(EZ_ANSI_COLOR_RED "[EZ2D ERROR]: Application is null. First, register your application using ezRegisterAsApplication(), then run ezStart()\n");
        return;
    }
    app->init = init;
}
...
void ezStart() {
    EZ_DEBUGC_RAW(EZ_ANSI_BOLD EZ_ANSI_COLOR_BLUE "[Easy2D]: " EZ_ANSI_COLOR_GREEN
